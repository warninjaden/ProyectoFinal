...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalle_pedidos`(`pedido_id`, `arma_id`, `precio`, `cantidad`) 
        VALUES (:pedido_id,:arma_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedido_id" => $_params['pedido_id'],
            ":arma_id" => $_params['arma_id'],
    ...
    public function mostrar(){
        $sql = "SELECT p.id,nombre,apellidos,email,total,fecha FROM pedidos as p
        INNER JOIN clientes as c ON p.cliente_id = c.id ORDER BY
        p.id DESC";
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        $sql = "SELECT 
                ...
                INNER JOIN armas pe ON pe.id= dp.arma_id
